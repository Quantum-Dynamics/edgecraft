...
def show_energy_graphs():
    ...
    axes[2].plot(basic.calc_velocity_along_edge(
        energy, const.e, const.B_0,
        basic.find_edge(energy, const.E_F, const.U_fluc, config.bulk),
...
def make_animation():
    ...
    def plot_anim(index: int) -> None:
        ...
        axes[1].axhline(const.E_F, color="black",
                        linestyle="dashed", label="$E_F$")
        axes[1].axhspan(const.E_F - const.U_fluc, const.E_F + const.U_fluc,
                        color="red", alpha=0.3)
        axes[1].hlines(1.5, 130, 130 + 10e-6 / const.unit_length,
                       color="black", linewidth=3)
    ...
    ani = animation.FuncAnimation(fig, plot_anim, interval=100,
                                  frames=len(config.gate_potential))
    ani.save(filename='PoC.gif', writer='pillow', dpi=300)
    plt.close()

    # Since time step 0 is run twice to generate the initial figure
    ds = ds[1:]
    velocity = velocity[1:]
